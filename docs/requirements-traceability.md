...
|---|---|---|---|---|---|
| R-01 | Load and render supplied URDF in browser | PDF Phase 1 | RobotModelAdapter, R3F scene | Adapter parse test; dev-server serves URDF (200) | Gate 1 ✅ |
| R-02 | Show live joint angles | PDF Phase 1 | TelemetryPanel + robotStore | Joint metadata test; telemetry table | Gate 1 ✅ |
| R-03 | Show live end-effector position | PDF Phase 1 | RobotModel useFrame → store.tcp | Zero-pose TCP test (~0,0,1.497) | Gate 1 ✅ |
| R-04 | Render six-key panel from JSON | PDF Phase 1, JSON | KeyPanel + keyConfig (Zod) | keyConfig + coordinate tests | Gate 1 ✅ |
...
| R-12 | Shared motion-control pipeline | PDF architecture language | Command/safety/runtime | Source audit and integration tests | Partial — RobotModelAdapter setter boundary established (Gate 1); pipeline in Gate 4 |
